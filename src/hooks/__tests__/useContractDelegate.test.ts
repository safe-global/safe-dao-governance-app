...
describe('_getContractDelegate()', () => {
  ...
  it('ignore the ZERO_ADDRESS as delegate', async () => {
    ...
    mockCall.mockImplementation((transaction) => {
      ...
      expect(transaction.data?.toString().toLowerCase()).toContain(SAFE_ADDRESS.toLowerCase().slice(2))

      expect(transaction.data?.toString().toLowerCase()).toContain(delegateIDInBytes.toLowerCase().slice(2))
