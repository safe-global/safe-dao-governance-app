...
describe('useAmounts', () => {
  ...
  it('vested amount is smaller than amount claimed', () => {
    ...
    expect(result.result.current).toEqual([parseEther('0').toString(), parseEther('490').toString()])
  })
  ...
  it('vesting half vested', () => {
    ...
    const result = renderHook(() => useAmounts(vestingClaim))
    expect(result.result.current).toEqual([parseEther('500').toString(), parseEther('500').toString()])
  })

  it('vesting half vested and quarter claimed', () => {
    const vestingClaim = createMockVesting(2, 1000, -1, 250)
    const result = renderHook(() => useAmounts(vestingClaim))
    expect(result.result.current).toEqual([parseEther('250').toString(), parseEther('500').toString()])
  })
  ...
  it('test polling and updating of vesting', () => {
    ...
    expect(result.result.current).toEqual([parseEther('0').toString(), parseEther(ONE_WEEK.toString()).toString()])
