...
export const useSummarizedLockHistory = (
...
): { totalLocked: BigNumber; totalUnlocked: BigNumber; totalWithdrawable: BigNumber } => {
  ...
  const totalWithdrawable = useMemo(() => {
    ...
    return withdrawableUnlocks.reduce((prev, event) => prev.add(event.amount), BigNumber.from(0))
  }, [lockHistory])

  return { totalLocked, totalUnlocked, totalWithdrawable }
}
