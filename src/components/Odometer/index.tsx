...
export const Odometer = ({ value = 0, decimals = 2 }: { value: number; decimals: number }): ReactElement => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', lineHeight: '2em' }}>
      {targetString.split('').map((digit, idx) => {
        ...
        return <RollingNumber key={`number-${idx}`} direction={direction} target={Number(targetDigit)} />
      })}
    </div>
  )
}
...
const RollingNumber = ({ direction, target }: { direction: number; target: number }): ReactElement => {
