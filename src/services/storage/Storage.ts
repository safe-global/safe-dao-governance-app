...
class Storage {
  ...
  public getItem = <T>(key: string): T | null => {
    ...
    try {
      ...
      console.error(`Failed to read "${key}" from local/session storage - ${(err as Error).message}`)
    }
  ...
  public removeItem = (key: string): void => {
    ...
    try {
      ...
      console.error(`Failed to remove "${key}" from local/session storage - ${(err as Error).message}`)
    }
  }
}
